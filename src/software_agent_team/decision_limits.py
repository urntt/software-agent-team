...
DECISION_LIMIT_REGISTRY: tuple[DecisionLimitDefinition, ...] = (
    ...
    _definition(
        "infrastructure.invocation-initialization-no-progress",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.execution:DEFAULT_INITIALIZATION_NO_PROGRESS_SECONDS",
        owner="OpenClawSubprocessExecutor initialization observer",
        rationale="Attributable OpenClaw process activity and startup checkpoints separate an inactive launch from provider generation without imposing a work-time budget or granting readiness prematurely.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="No attributable process activity or checkpoint progress enters a visible diagnostic window and eventually stops only the exact SAT-owned invocation.",
        recovery="Inspect the last attributable initialization checkpoint and environment evidence before revising this infrastructure ceiling.",
        evidence="tests/test_execution.py initialization liveness coverage",
    ),
    _definition(
        "infrastructure.invocation-initialization-grace",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.execution:DEFAULT_INITIALIZATION_STALL_GRACE_SECONDS",
        owner="OpenClawSubprocessExecutor initialization observer",
        rationale="A visible final window permits late attributable process activity or a startup checkpoint to recover before exact-process termination.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="Process-activity or checkpoint recovery resumes initialization; continued inactivity becomes initialization_stalled.",
        recovery="Use checkpoint timing evidence from supported hosts before changing the warning window.",
        evidence="tests/test_execution.py initialization grace recovery coverage",
    ),
    _definition(
        "infrastructure.invocation-initialization-process-sample",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.execution:resolve_initialization_process_sample_interval",
        owner="OpenClawSubprocessExecutor initialization observer",
        rationale="Identity-bound process sampling must be frequent enough to observe activity during grace without traversing the process tree on every liveness poll.",
        visibility=DecisionLimitVisibility.DETAILED,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="The derived interval changes observation latency only; process activity cannot grant readiness or extend a user deadline.",
        recovery="Tune against initialization activity and inactivity fixtures, then rerun exact-process and decision-limit tests.",
        evidence="tests/test_execution.py active initialization coverage",
    ),
    _definition(
    ...
    _definition(
        "infrastructure.response-finalization-no-progress",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.execution:DEFAULT_RESPONSE_FINALIZATION_NO_PROGRESS_SECONDS",
        owner="OpenClawSubprocessExecutor response-finalization observer",
        rationale="A terminal assistant record ends provider generation, but the runtime still needs a distinct bounded path to serialize its result and exit.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="Observable process output renews finalization; sustained post-response inactivity stops only the exact invocation.",
        recovery="Inspect finalization progress and pinned-runtime teardown evidence before revising this infrastructure ceiling.",
        evidence="tests/test_execution.py response-finalization coverage",
    ),
    _definition(
        "infrastructure.response-finalization-grace",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.execution:DEFAULT_RESPONSE_FINALIZATION_STALL_GRACE_SECONDS",
        owner="OpenClawSubprocessExecutor response-finalization observer",
        rationale="A visible diagnostic window lets late result-serialization output recover without extending the provider lease.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="Recovered output continues the same finalization; continued inactivity becomes response_finalization_stall.",
        recovery="Use supported-host finalization evidence before changing the warning window.",
        evidence="tests/test_execution.py response-finalization recovery coverage",
    ),
    _definition(
    ...
    _definition(
        "infrastructure.openclaw-config-validation-timeout",
        _INFRASTRUCTURE,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.runtime_configuration:CONFIG_VALIDATION_TIMEOUT_SECONDS",
        owner="OpenClaw runtime configuration validation",
        rationale="A cold OpenClaw configuration load can take longer than an ordinary local command on a supported low-memory host, while still requiring a finite pre-Agent boundary.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.MAINTAINER_POLICY,
        consequence="The named configuration check stops before any Agent or provider request starts.",
        recovery="Inspect the named timeout diagnostic, restore host capacity if needed, and rerun Planning admission.",
        evidence="tests/test_runtime_configuration.py named configuration timeout coverage",
    ),
    _definition(
    ...
    *tuple(
    ...
    _definition(
        "protocol.response-correction-fields",
        _PROTOCOL,
        DecisionValueReferenceKind.PYTHON_ATTRIBUTE,
        "software_agent_team.response_corrections:MAX_CORRECTION_FIELDS",
        owner="response correction compiler",
        rationale="A field-targeted correction must remain finite, auditable, and distinguishable from whole-object regeneration.",
        visibility=DecisionLimitVisibility.ON_TRIGGER,
        configurability=DecisionLimitConfigurability.NOT_CONFIGURABLE,
        consequence="An over-wide validation failure is rejected without partial normalization or a model correction call.",
        recovery="Reduce the model-owned response contract or split the semantic submission at an owned protocol boundary.",
        evidence="tests/test_response_corrections.py overflow and authority-boundary coverage",
    ),
    _definition(
