...
def build_semantic_repair_request(
    ...
    if request.capability is AgentCapability.PLANNING:
        ...
        capability_check = (
            ...
            "the same criterion ID. Repeat only tool checks whose evidence is "
            "missing or changed; integrity-checked results from an earlier attempt "
            "in this bounded repair chain remain eligible. tool_evidence may "
            "contain only a bounded exact output fragment from an eligible result; "
            "do not predict a controller attempt or tool ID or echo controller-owned "
            "tool names or outcomes. Every fragment must match at least one eligible "
            "result; the controller binds every match with its attempt and tool ID "
            "and deduplicates repeated or overlapping fragments. Use "
