...
def build_semantic_repair_request(
    ...
    if request.capability is AgentCapability.PLANNING:
        ...
        capability_check = (
            ...
            "tool_evidence may contain only a bounded exact output fragment "
            "from this invocation; do not predict a controller tool ID or echo "
            "controller-owned tool names or outcomes. Every fragment must match "
            "at least one current result; the controller binds every match and "
            "deduplicates repeated or overlapping fragments. Use "
