...
| Use invocation-bound typed submission for Planning and dynamic Agent semantics | A model that already completed valid work can still wrap, truncate, duplicate, or corrupt a JSON-shaped assistant message. Every invocation therefore receives a fresh binding through the isolated `sat_submit_artifact` plugin, and the controller accepts only one final successful attributable call whose canonical arguments match the private envelope. Dynamic Agent tools expose their exact semantic schema. Adaptive Planning instead separates a permissive object-only transport schema from the exact controller-bound semantic schema: this guarantees that a syntactically structured proposal or correction reaches Controller validation, where harmless forbidden fields can be recorded and removed and model-owned defects can receive targeted correction. The private envelope remains bound to the exact semantic schema digest. Missing, duplicate, non-final, malformed, failed, unsafe, or mismatched submissions fail closed; assistant text remains evidence but has no semantic authority. During targeted correction the model submits only `replacement_values`, while the Controller alone retains the base-response identity and ordered target paths. Each slot exposes its exact value schema and validator-owned constraints; relational ProductDefinition defects are collected across independent dimensions and each dimension is replaced atomically so fail-fast ordering cannot consume one repair per dependent field. Model-facing Planning context contains only project facts needed for the decision; run identity, destination, route, authorization, and timestamps remain Controller-owned. |
