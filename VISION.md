...
| Use invocation-bound typed submission for dynamic Agent artifacts | A model that already completed valid work can still wrap, truncate, duplicate, or corrupt a JSON-shaped assistant message. Each dynamic invocation therefore receives an exact controller-frozen JSON Schema and fresh binding through the isolated `sat_submit_artifact` plugin. The controller accepts only one final successful attributable tool call whose canonical arguments match the private invocation envelope; missing, duplicate, non-final, malformed, failed, unsafe, or mismatched submissions fail closed. Assistant text remains evidence but has no artifact authority. Semantic models, controller fields, Git and Review evidence rules remain separately enforced, and the same tool narrows to an exact slot-only schema during targeted correction. |
...
| Treat contract-ineligible JSON arrays as presentation on text-compatibility calls | Bootstrap Planning and fixed-role compatibility responses must contain one top-level object. An argv array in explanatory text cannot satisfy or compete with that contract, so rejecting it wastes a repair call and can create a secondary failure. The parser still rejects any additional object, including one nested in an array, and preserves raw transport output. Dynamic execution artifacts use typed submission instead. |
