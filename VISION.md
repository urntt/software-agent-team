...
| Decision | Reason |
| --- | --- |
| Return missing user authority to dialogue rather than proposal repair | A model cannot fill a decision that belongs to the user. When validation of a complete proposal discovers a missing target-user, primary-workflow, or delivery-maturity decision, each invariant retains its typed authority and the Controller requests one question-only response constrained to that exact ProductDefinition dimension. Mixed model/user diagnostics grant no proposal replacement path; model-owned siblings are checked again against the next complete proposal after the ordinary dialogue records an answer. This state transition avoids both an unauthorized inference and a retry loop that can never change `kind=proposal` into user input. |
