...
| Return missing user authority to dialogue and settle Planning state consistently | A rejected proposal cannot model-correct a material decision that only the user may make. Each validation invariant retains its own failure class and authority; if a proposal still lacks a user-owned ProductDefinition decision, the Controller requests one atomic question-only response for that dimension, records the answer in the normal transcript, and then revalidates the whole proposal, including any independent model-owned defects. A terminally invalid initial Planning attempt becomes `failed`. A completed invalid natural-language revision restores the preceding valid proposal so it remains reviewable and approvable; an executor failure, cancellation, or interruption retains its own terminal state instead. |
