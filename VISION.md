...
| Use invocation-bound typed submission for Planning and dynamic Agent semantics | A model that already completed valid work can still wrap, truncate, duplicate, or corrupt a JSON-shaped assistant message. Every Adaptive Planning and dynamic execution invocation therefore receives an exact controller-frozen JSON Schema and fresh binding through the isolated `sat_submit_artifact` plugin. The controller accepts only one final successful attributable tool call whose canonical arguments match the private invocation envelope; missing, duplicate, non-final, malformed, failed, unsafe, or mismatched submissions fail closed. Assistant text remains evidence but has no semantic authority. Semantic models, controller fields, Git and Review evidence rules remain separately enforced. During targeted correction, the same tool narrows to an exact value-only slot schema: the model submits `replacement_values`, while the Controller alone retains the base-response identity and ordered target paths. |
