...
| Keep profile criterion definitions controller-owned while permitting explicit task bindings | An execution profile owns its fixed acceptance text and verification contract, so Planning must not duplicate or rewrite them. Writer tasks may still identify relevant profile obligations, while quality tasks may state verification focus. The policy-aware controller therefore validates task references against proposal-owned plus current profile IDs, preserves valid bindings in the approved plan, and rejects any other ID before Agent creation. |
