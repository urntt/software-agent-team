...
| Give Reviewer the minimum coherent probe capability | Source remains read-only and network, background processes, project edits, and Agent spawning remain denied. Reviewer may create only temporary probe scripts under the sandbox's writable `/tmp` and execute simple foreground commands; the pinned runtime includes `uv` so exact generated-project commands are observable instead of merely promised. |
