...
| Treat terminal failure as evidence | Provider, sandbox, artifact, budget, and convergence failures must remain observable instead of being retried or discarded silently. Cost accounting is an independent evidence dimension: unknown usage remains unknown and prevents another user-budget call, but it cannot reclassify an already-attributed runtime or response failure. A violation introduced by one completed call belongs to that call; another already-reserved parallel invocation may settle its own known usage without inheriting that rejection, while every later reservation still fails closed. When no earlier failure exists, a post-call budget rejection remains the primary resource-limit reason. |
