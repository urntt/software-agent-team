...
| Bind each ProductDefinition question to one declared decision bundle | A free-text answer authorizes only the ProductDefinition dimensions visibly declared by its question, not every nearby missing field. Tightly coupled audience, workflow, and maturity choices may share one question when every option resolves the full bundle; unrelated choices remain separate, and an ambiguous or undeclared value returns to focused clarification. The Controller projects exact transcript answers into resolved statement fields rather than relying on the model to reproduce user wording. Question text, rationale, missing evidence, consequences, options, and the declared bundle are one semantic correction unit: a defect replaces the complete question rather than relabeling preserved wording through one field patch. Current submissions contain every authority key, reject unknown keys and duplicate/out-of-range dimensions, and retain backward-readable persisted evidence. This reduces redundant dialogue without widening authority or depending on model intelligence for exact provenance. |
