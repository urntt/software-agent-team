...
| Keep Tester and Reviewer independent, with configurable dispatch concurrency | They inspect the same immutable evidence and never consume each other's interpretation. Parallel dispatch reduces elapsed time when provider and host capacity permit; known Linux/cgroup memory or finite PID headroom may visibly lower only the run-scoped ceiling before Planning, without raising or rewriting the user's saved ceiling. Serial dispatch prevents overload without changing the semantic experiment. |
