...
| Compile every model route from one frozen transport-neutral profile | A `provider/model` string alone cannot distinguish an OpenClaw-native route, a custom OpenAI or Anthropic endpoint, or local Ollama, and consumer-specific compatibility branches allow a route to save successfully but fail later. Each saved `ModelProfile` therefore embeds one versioned, secret-free runtime contract and canonical digest covering provider-native identity, explicit transport, endpoint policy, credential reference, model limits, and required capabilities. Startup inspection, provider smoke, Planning, dynamic execution, telemetry, and reports consume that same frozen profile. Reviewed presets may bridge a pinned-catalog lag, but cannot copy credentials, infer an undeclared transport, enable fallback, or create a model-name branch in one consumer. |
