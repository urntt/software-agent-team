...
| Keep saved user defaults and model profiles secret-free | The product wizard may store provider/model references, routing preferences, discovered or user-supplied price/context metadata, concurrency, and progress visibility, but provider credentials remain in SAT's isolated OpenClaw-owned state and never enter SAT control-plane configuration or exports. A later launch revalidates the exact saved profile through an effective isolated configuration, which may be temporary; the incidental presence of a persistent OpenClaw config is not an onboarding sentinel. Only missing configuration, an unavailable saved bootstrap route, or explicit `sat configure` enters configuration dialogue, and an unrelated discovered default never outranks the saved model. Per-task USD authority and an optional deadline are asked at task admission rather than persisted as hidden global defaults. |
