...
| Use measured capability defaults, controller-resolved Adaptive timeouts, independent scope floors, and bounded-repair timeouts | Planning, implementation, and verification have different measured workloads. Fixed-fixture compatibility defaults are 120 seconds for Clarifier, 180 for Planner, 900 for coding/integration, and 300 for testing/review. Adaptive Planning classifies each Agent workload as routine, substantial, or complex; it does not authorize seconds. Product policy maps that estimate into a capability-specific default-to-ceiling envelope. Exact Review scope independently maps fewer than 6, 6–10, or 11+ criteria to routine, substantial, or complex and supplies a minimum. The 11-criterion complex boundary follows a provider-backed timeout at the substantial allowance; the separate 10-criterion timeout only proved that routine was insufficient, so it remains substantial rather than receiving an unverified increase. An explicit user override may select an exact value only inside the effective envelope, and the controller records and enforces the result. A repair receives the same per-invocation timeout because it regenerates a complete semantic response. Repair remains limited to one call, while total call count, Agent duration, token, and cost budgets bound the complete run. |
