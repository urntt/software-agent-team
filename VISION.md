...
| Keep expected paths advisory | A Planner can forecast repository paths to make task scope understandable, but that forecast cannot become another requirement, permission boundary, completion checklist, or generated-project manifest. Agents therefore never create, modify, inspect, or track a path solely because it appears in `expected_paths`; the TaskBrief, execution profile, Agent permission, workspace scope, repository ignore policy, and command manifest remain authoritative. This avoids turning an ignored local setup artifact into a contradictory delivery obligation. |
